...
data (XYZ), then 2 bytes temperature data.
...
void addLSM330Msg(byte i)
{
  ...
  byte temp[6];
  readLSM330(LSM330_OUT_X_L_G, cs_g_pin[i], 6, temp);
  // Need to switch high/low byte order
  data_buffer[data_size++] = temp[1];
  data_buffer[data_size++] = temp[0];
  data_buffer[data_size++] = temp[3];
  data_buffer[data_size++] = temp[2];
  data_buffer[data_size++] = temp[5];
  data_buffer[data_size++] = temp[4];

  // Add 6 bytes of accelerometer data
  readLSM330(LSM330_OUT_X_L_A, cs_a_pin[i], 6, temp);
  // Need to switch high/low byte order
  data_buffer[data_size++] = temp[1];
  data_buffer[data_size++] = temp[0];
  data_buffer[data_size++] = temp[3];
  ...
  data_size++;
  // Sign extend by 1 byte
  data_buffer[data_size++] = data_buffer[data_size-1];
  data_buffer[data_size-2] = (data_buffer[data_size-1] & 0x80 ? 0xFF : 0x00);
}
...
void addMPU6050Msg(byte i, byte addr)
{
  ...
  // Add 6 bytes accelerometer data
